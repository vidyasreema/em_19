...
class CatalogConfig(models.Model):
    ...
    # --- Cover page text ---
    cover_title = fields.Text(
        string="Cover Title", default="PRODUCT\nCATALOG",
        help="Big title on the cover. Press Enter for a new line.")
    cover_tagline = fields.Char(
        string="Cover Tagline", default="PREMIUM SELECTION \u2022 2026")

    # --- Grid page heading ---
    grid_title = fields.Char(string="Collection Title",
                             default="PREMIUM COLLECTION")
    grid_subtitle = fields.Char(string="Collection Subtitle",
                                default="SELECTED CUTS \u2022 GRASS-FED")

    # --- Thank-you page ---
    thanks_title = fields.Text(
        string="Thank-You Message", default="WE'D BE GLAD\nTO SERVE YOU",
    ...
    # --- Contact block on the thank-you page ---
    ...
    delivery_note = fields.Char(string="Delivery Note",
                                default="FREE DELIVERY \u2022 MIN AED 300")
    ...
    def _compute_qr_image(self):
        ...
        for rec in self:
            if qrcode and rec.qr_url:
                ...
                qr.make(fit=True)
                img = qr.make_image(fill_color="#2c0e16",
                                    back_color="#f0e6d2").convert("RGB")
                buf = BytesIO()
